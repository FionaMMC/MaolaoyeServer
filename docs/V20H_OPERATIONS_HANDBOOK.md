...
### 2.1 选股 + 调仓（日内时序，**T 日傍晚 trigger 算 T+1 信号**）

```
T-1 日 18:00 → 拉到的 T 日 orders 已经就绪
                                                ↓
T 日 09:10 集合竞价 → client 提交 T 日 orders 给 QMT
                                                ↓
T 日 09:30 开盘 → QMT 撮合
                                                ↓
T 日 15:00 收盘
                                                ↓
T 日 15:10 → client 推 trade_result（先 settle 当日成交）
            server settlement: 扣手续费/印花税、更新 instance_state、写 perf_snapshot
                                                ↓
T 日 15:30 → client 推 EOD OHLCV（给其他策略用，V20H 直接读 pred 不依赖）
                                                ↓
T 日 16:00 → client 跑 trigger_pipeline.py（默认下一交易日 T+1）
            server 跑 V20H adapter:
            ...
            output: orders 表里写好 T+1 的 PENDING 订单
                                                ↓
T 日 18:00 → 你/搭档拉 /orders?date=T+1 眼检（5 分钟，发现问题随时改）
                                                ↓
T+1 日 09:10 → 重复以上 → ... ↻
```

**为什么不用早上 trigger**：早上 09:00 trigger 到 09:15 集合竞价只有 15 分钟，万一 pipeline 报错来不及修。下午 16:00 trigger 给自己留 **17 小时缓冲**，且数据新鲜度完全等价（pred 周更，OHLCV T 收盘后才有，settlement 15:10 已完成）。
...
> ✅ **修复已 deploy**（5/16 22:19）+ **5/18 信号已生成**（5/17 16:00 你 trigger 的，19 条 SELL，0 BUY；
> 见 `/admin/orders?date=20260518`）。周一只需要正常执行 + 收尾验证。
...
# 关键字段：
#   pred_status.lag_hours  → 应该 < 168（7 天内）；当前 lag 12 天，OK
#   blacklist.merged_total → 应该 ~63
#   instances[0].virtual_cash → ~434K
#   instances[0].holdings_count → 852

# 2. 看 5/18 已就绪的 19 条 PENDING（5/17 16:00 trigger 的产物）
curl -sH "Authorization: Bearer $QMT_API_KEY" \
  "http://120.26.138.82:8000/admin/orders?date=20260518&status=PENDING" | jq

# 期望：19 条全是 SELL（12 cap_weight 微调 + 7 完全清仓），0 BUY
# 关键确认：
#   - 没有 688/689 前缀（科创板已过滤）
#   - 没有 688498.SH（修复前那 4 条旧单已被 5/17 trigger 时的幂等清掉）
```

### 5.2 周一 9:10 — 集合竞价：客户端下单

搭档侧（Windows）：
```powershell
# 先确认拉到了 5-bug 修复的最新代码
cd C:\parttime\MaolaoyeServer && git pull origin master

# 下单：order_submit.py 会拉 /orders?date=20260518，按 SELL-first 排序提交到 QMT
.\venv\Scripts\python.exe v2.3\client\order_submit.py
```

预期：19 个 SELL 大部分能成（集合竞价的卖单一般都能撮合，除非票停牌）。

### 5.3 周一 15:10 — 收盘后回报推送
...
服务器自动：
- settlement 更新 instance_state（**含手续费 + 印花税**，Bug B 修复后扣对了）
- 写 perf_snapshot
- **如果有"防穿仓静默拒绝"会触发 `bookkeeping_divergence` flag** ← 必须当天查

### 5.4 周一 15:30 — 推 OHLCV（其他策略用）

```powershell
.\venv\Scripts\python.exe v2.3\client\data_collector.py
```

V20H 自己不用 ctx.market（直接读 pred_today.close），但 server 别的策略会用。

### 5.5 周一 16:00 — trigger 算 T+1（周二 5/19）信号
...
# 或显式：
.\venv\Scripts\python.exe v2.3\client\trigger_pipeline.py --date 20260519
```

预期：周二的 signals 通常 < 5 条（cap_weight 微调），因为下次主调仓在 7 月中（di=1274）。

### 5.6 周一 18:00 — 验证一切

```bash
# 1. 当天 NAV 是否合理（卖了 ~470K 股票收回现金 → cash 应到 ~900K）
curl -sH "Authorization: Bearer $QMT_API_KEY" \
...
# 3. 策略状态：第一次跑后应该看到 last_rb_idx=1232（5/18 那天的 di）
curl -sH "Authorization: Bearer $QMT_API_KEY" \
  "http://120.26.138.82:8000/admin/strategy-state" | jq

# 4. 周二 5/19 的 PENDING（5.5 步 trigger 的产物）
curl -sH "Authorization: Bearer $QMT_API_KEY" \
  "http://120.26.138.82:8000/admin/orders?date=20260519&status=PENDING" | jq '.data.items | length'
# 期望：< 5 条（非 rebal 日，仅 cap_weight 微调）
```
...
### 6.1 每日（每个交易日 T）— **正式标准时间线**

> 设计原则：**「T 日傍晚算 T+1 信号」**——给自己留 17 小时缓冲，比早上 trigger 抗故障得多。

| 时间 | 谁 | 做什么 | 备注 |
|---|---|---|---|
| 09:10 | 搭档 | `order_submit.py` → 提交 T 日 orders | 信号是 T-1 日 16:00 算好的 |
| 09:30 | (market) | QMT 撮合开盘 | — |
| 15:00 | (market) | 收盘 | — |
| 15:10 | 搭档 | `trade_result_push.py` → 推 trade_result | **先 settle 才能算后面的状态** |
| 15:30 | 搭档 | `data_collector.py` → 推 EOD OHLCV | V20H 不直接用，但 server 留着给其他策略 |
| **16:00** | **搭档** | **`trigger_pipeline.py`** → **算 T+1 信号** | 默认下一交易日；服务器幂等 |
| 18:00 | 你 | `curl /admin/orders?date=T+1` 眼检 | 看一下信号合理不，发现问题随时再 trigger 一次 |
| 22:00 | (sleep) | — | 周日 22:00 顺便跑一次 `bash daily_v20h.sh` 刷 pred |

每日**必看**（你 18:00 那次眼检）:
1. `bookkeeping_divergence` 总数 — **必须 = 0**，非 0 当天就要人工对账
2. NAV 是否在合理范围（vs CSI1000）
3. holdings_count 是否稳定（首次 deploy 后会从 852 → ~845 收敛，然后保持）
4. T+1 信号数量是否合理：非 rebal 日通常 < 5 条；rebal 日（每 42 天）才会几十-几百条

**为什么必须每天 trigger（即使 42 天才主调仓）**：
- V20H 每天要更新 `equity_history` / `daily_rets`（vol_target 输入；跳天会漏数据点）
- 每天 `apply_cap_weight` 微调超权重（典型 0-3 笔小 SELL）
...
每次 pipeline 跑（每个交易日 T 的 16:00，给 T+1 算信号）：
...
# 09:10 集合竞价下单（信号来自昨天 16:00 trigger）
.\venv\Scripts\python.exe v2.3\client\order_submit.py

# 15:10 推 trade_result
.\venv\Scripts\python.exe v2.3\client\trade_result_push.py

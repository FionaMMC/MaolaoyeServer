...
## 5b. 自动化每日运行（关键 —— 不自动化 = v79 影子时钟不走）

**手动跑一次不够。** v79 上线前要影子对账**连续 3 个交易日** `consistent=True`。每天不推、时钟就不走——实测：周一手动推过一次(=day1),周二没推 → **卡住不动**。所以必须挂成**每交易日自动跑**。

用 Windows **任务计划程序**,周一到周五收盘结算后(如 16:30)自动跑 `run_reconcile.bat`:

```bat
schtasks /Create /TN "QMT_Reconcile_Push" /TR "C:\path\to\run_reconcile.bat" /SC WEEKLY /D MON,TUE,WED,THU,FRI /ST 16:30 /F
```

- `/ST 16:30`:收盘(15:00)+ **当天成交结算完成之后**——确保 QMT 持仓已反映当天成交。你的结算更晚就往后调。
- 周六日不跑(无新数据);节假日跑也无害(对账幂等,只是重报同一状态)。
- 【CONFIRM】把 `C:\path\to\` 换成 `run_reconcile.bat` 的实际路径。任务需在**能连 QMT 的用户上下文**下运行——建议先用"仅在用户登录时运行"跑通,再视情况改"无论是否登录"。
- 建好后**立即验证一次**:`schtasks /Run /TN "QMT_Reconcile_Push"`,然后让服务器侧确认日志出现 `shadow_compare: consistent=True`。

> **三条 Windows 链都要挂成自动**(pred 刷新 15:00 / 篮子周更 / 对账推送 16:30),任何一条断了都**静默无告警**——这是反复踩过的坑。对账推送是当前唯一没自动化、正卡住 v79 的那条,优先挂上。

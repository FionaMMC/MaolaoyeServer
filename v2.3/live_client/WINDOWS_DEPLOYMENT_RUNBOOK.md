...
Use `preflight`, `submit` or `settle` for the corresponding task. Replace
...
4. T+1 close `settle`: read terminal QMT results and return them to the server. If
   the server is unavailable, retain local evidence and retry settlement later;
   never resubmit the orders.
